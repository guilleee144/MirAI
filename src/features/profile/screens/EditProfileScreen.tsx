...
// Migrated from app/(app)/edit-profile/index.tsx.
// expo-image-picker -> react-native-image-picker: launchImageLibraryAsync({mediaTypes,
// allowsEditing, aspect, quality}) returning {canceled, assets} becomes
// launchImageLibrary({mediaType:'photo', quality}) returning {didCancel, assets}.
// react-native-image-picker has no built-in cropping, so allowsEditing/aspect are
// dropped here (follow-up: add a cropper lib later if 1:1 cropping is required).
...
export function EditProfileScreen() {
  ...
  const handlePickPhoto = async () => {
    ...
    try {
      ...
      const contentType = asset.type ?? `image/${fileExt === 'jpg' ? 'jpeg' : fileExt}`

      // React Native's correct way to upload files
    ...
    <View style={{ flex: 1, backgroundColor: '#0B0F17' }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 56, paddingBottom: 24, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          ...
          <Text style={{ color: '#fff', fontSize: 22, fontWeight: '800', letterSpacing: -0.5 }}>Edit Profile</Text>
        </View>
          ...
          <TouchableOpacity onPress={handlePickPhoto} activeOpacity={0.8}>
            <View style={{ position: 'relative' }}>
              {avatarUrl ? (
                ...
                >
                  <Text style={{ color: '#fff', fontSize: 36, fontWeight: '800' }}>{username_initial}</Text>
                </LinearGradient>
              )}
              ...
              >
                {isUploadingPhoto ? <ActivityIndicator color="#fff" size="small" /> : <Text style={{ color: '#fff', fontSize: 12 }}>✎</Text>}
              </View>
            </View>
          </TouchableOpacity>
          <Text style={{ color: '#7B8496', fontSize: 12, marginTop: 10 }}>{isUploadingPhoto ? 'Uploading...' : 'Tap to change photo'}</Text>
        </View>
        ...
        <View style={{ paddingHorizontal: 20, gap: 20 }}>
          {/* Username */}
          <View>
            <Text style={{ color: '#B8C1D1', fontSize: 13, fontWeight: '500', marginBottom: 8 }}>Username</Text>
            <TextInput
              value={username}
          ...
          {/* Display name */}
          <View>
            <Text style={{ color: '#B8C1D1', fontSize: 13, fontWeight: '500', marginBottom: 8 }}>Display name</Text>
            <TextInput
              value={displayName}
          ...
          {/* Bio */}
          <View>
            <Text style={{ color: '#B8C1D1', fontSize: 13, fontWeight: '500', marginBottom: 8 }}>Bio</Text>
            <TextInput
              value={bio}
              onChangeText={setBio}
              style={{
              ...
              maxLength={160}
            />
            <Text style={{ color: '#7B8496', fontSize: 11, marginTop: 4, textAlign: 'right' }}>{bio.length}/160</Text>
          </View>

          {/* Current mood */}
          <View>
            <Text style={{ color: '#B8C1D1', fontSize: 13, fontWeight: '500', marginBottom: 12 }}>Current mood</Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {MOODS.map((mood) => {
                const selected = selectedMood === mood.id
                return (
                  <TouchableOpacity key={mood.id} onPress={() => setSelectedMood(selected ? null : mood.id)} activeOpacity={0.8}>
                    <View
                      style={{
                        flexDirection: 'row',
                        alignItems: 'center',
                    ...
                    >
                      ...
                      <Text style={{ color: selected ? '#fff' : '#B8C1D1', fontSize: 13, fontWeight: selected ? '600' : '400' }}>{mood.label}</Text>
                    </View>
                  </TouchableOpacity>
                )
              })}
          ...
          <TouchableOpacity onPress={handleSave} disabled={isSaving} activeOpacity={0.85} style={{ marginTop: 8 }}>
            ...
            >
              {isSaving ? <ActivityIndicator color="#fff" /> : <Text style={{ color: '#fff', fontSize: 16, fontWeight: '600' }}>Save changes</Text>}
            </LinearGradient>
          </TouchableOpacity>
        </View>
      </ScrollView>
