  ...
  return (
    <Svg width={RADAR_SIZE} height={RADAR_SIZE}>
      {gridLevels.map(level => {
        const gridPoints = entries.map((_, i) => getRadarPoint(i * angleStep, level, RADAR_RADIUS))
        const gridStr = gridPoints.map(p => `${p.x},${p.y}`).join(' ')
        return <Polygon key={level} points={gridStr} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="1" />
      })}
      {entries.map((_, i) => {
...
useFocusEffect(
  ...
  const loadProfile = async () => {
    if (!user) return
    setIsLoading(true)
    const [{ data: profileData }, { data: tasteData }] = await Promise.all([
