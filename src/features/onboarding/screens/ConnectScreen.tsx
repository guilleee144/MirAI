// src/features/onboarding/screens/ConnectScreen.tsx
// Migrated from app/(auth)/onboarding/connect.tsx.
// The old router.replace('/(app)') is gone — finishing now calls
// markOnboarded(), and RootNavigator reacts by swapping to AppStack.
...
export function ConnectScreen() {
  ...
  const handleFinish = async () => {
    ...
    const { error } = await supabase.from('profiles').update({ is_onboarded: true }).eq('id', user.id)
      ...
      <View style={{ flex: 1, paddingTop: 64, paddingHorizontal: 24 }}>
        <View style={{ marginBottom: 40 }}>
          ...
          <Text style={{ color: '#B8C1D1', fontSize: 15, lineHeight: 22 }}>
            Optional but powerful. MirAI learns from your existing history to give you better recommendations from
            day one.
          </Text>
        </View>
        ...
        <View style={{ gap: 12, marginBottom: 40 }}>
          {CONNECTIONS.map((conn) => {
            ...
            return (
              <TouchableOpacity key={conn.id} onPress={() => handleConnect(conn.id)} activeOpacity={0.8}>
                <View
                  style={{
                    flexDirection: 'row',
                    alignItems: 'center',
                ...
                >
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
                    ...
                    >
                      <Text style={{ color: conn.color, fontSize: 11, fontWeight: '700' }}>{conn.initials}</Text>
                    </View>
                    <View>
                      <Text style={{ color: '#fff', fontSize: 15, fontWeight: '600' }}>{conn.name}</Text>
                      <Text style={{ color: '#7B8496', fontSize: 13, marginTop: 2 }}>{conn.desc}</Text>
                    </View>
                  </View>
                  ...
                  >
                    <Text style={{ fontSize: 12, fontWeight: '600', color: isConnected ? '#2EE59D' : '#7B8496' }}>
