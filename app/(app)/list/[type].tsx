    ...
    >
      <View style={{ borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' }}>
        ...
        <View style={{ padding: 8, backgroundColor: '#141826' }}>
          ...
          {media.genres[0] && (
            <View style={{ backgroundColor: 'rgba(108,92,231,0.12)', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2, alignSelf: 'flex-start', marginTop: 4 }}>
...
export default function ListScreen() {
  ...
  const listType = (type as ListType) ?? 'trending-anime'
  const config = LIST_CONFIG[listType] ?? LIST_CONFIG['trending-anime']

  const fetchFn = listType === 'trending-anime'
    ? getTrendingAnime
    : listType === 'trending-manga'
    ? getTrendingManga
    : getTopAnime
  ...
  const renderItem = useCallback(({ item }: { item: AniListMedia }) => (
    <MediaGridCard
      media={item}
      onPress={() => router.push(`/(app)/media/${item.id}?type=${item.type}` as any)}
    />
  ), [router])
