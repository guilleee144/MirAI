        ...
        <View style={{ padding: 8 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {item.content_type === 'game' && metadata?.playtime_hours > 0 && (
              <Text style={{ color: '#7B8496', fontSize: 9 }}>
                {metadata.playtime_hours}h
              </Text>
            )}
            {item.rating && (
    ...
    <View style={{ flex: 1, backgroundColor: '#0B0F17' }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} stickyHeaderIndices={[1]}>
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 60, paddingBottom: 16 }}>
          ...
          <Text style={{ color: '#7B8496', fontSize: 14, marginTop: 4 }}>
            Your anime, manga & gaming collection
          </Text>
          ...
          <View style={{ flexDirection: 'row', gap: 12, marginTop: 16 }}>
            {[
              ...
              <View key={stat.label} style={{ flex: 1, backgroundColor: '#141826', borderRadius: 12, padding: 10, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' }}>
        ...
        {isLoading ? (
          ...
          <View style={{ paddingHorizontal: 20, paddingTop: 60, alignItems: 'center' }}>
            ...
            <Text style={{ color: '#7B8496', fontSize: 14, textAlign: 'center', lineHeight: 20 }}>
              {activeType === 'game'
                ? 'Connect your Steam account to import your games'
                : 'Start discovering and adding content to your library'}
            </Text>
            <TouchableOpacity
              onPress={() => activeType === 'game'
                ? router.push('/(app)/connect-steam' as any)
                : router.push('/(app)/discover' as any)
              }
              style={{ marginTop: 20 }}
            >
              <LinearGradient
                  ...
                  <LibraryCard
                    ...
                    onPress={() => {
                      if (item.content_type !== 'game') {
                        router.push(`/(app)/media/${item.external_id}?type=${item.content_type.toUpperCase()}` as any)
                      }
                    }}
                  />
                </View>
